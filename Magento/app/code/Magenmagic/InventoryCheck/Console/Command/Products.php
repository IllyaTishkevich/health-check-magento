 ...
 * </pre>
 */
class Products extends \Symfony\Component\Console\Command\Command
{
    /**
    ...
    const INPUT_PRODUCT_IDS = 'product-ids';
    ...
    public function __construct(
        \Magenmagic\InventoryCheck\Helper\Products $helper, \Magento\Framework\App\State $state
    ) {
        $this->_state = $state;
        $this->helper = $helper;
        parent::__construct();
    ...
    {
        $this->setName('magenmagic:enable_products')->setDescription(__('Enable Products'))->setDefinition(
            $this->getOptions()
        );
        parent::configure();
    }
    ...
    protected function execute(
        ...
        try {
            ...
            if ($this->helper->isEnabled()) {
                ...
                $productIds          = explode(',', $input->getOption(self::INPUT_PRODUCT_IDS));
                $productIdsProcessed = [];
                if (sizeof($productIds) > 0) {
                    foreach ($productIds as $index => $item) {
                        if ($item != "") {
                            $productIdsProcessed[$index] = array('product_id' => $item);
                        }
                    }
                }
                ...
                foreach ($storeIds as $storeId) {
                    ...
                    $ids = $this->helper->enableProducts($storeId, $dryRun, $productIdsProcessed);
                    if (sizeof($ids) > 0) {
                        $output->writeln(
                            __('Processed store %2 ids #  %1', implode(',', array_unique($ids)), $storeId)
                        );
                    }
                }
            }
    ...
    protected function getOptions()
    {
        return [
            ...
            new InputOption(
                self::INPUT_PRODUCT_IDS,
                null,
                InputOption::VALUE_NONE | InputOption::VALUE_REQUIRED,
                'Comma separated products Ids. (i.e. 51519,51520,51521,)'
            ),
